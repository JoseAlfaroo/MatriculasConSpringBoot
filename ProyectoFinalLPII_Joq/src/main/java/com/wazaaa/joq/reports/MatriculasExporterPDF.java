...
public class MatriculasExporterPDF {
	...
	private void escribirDatosDeLaTablaMatriculasActivas(PdfPTable tabla) {
		...
		for(Matricula matricula : listaMatriculas) {
			...
			if(matricula.getUsuario().getId() == usuario.getId() && matricula.getEstado().equals("Activa")) {
	...
	private void escribirDatosDeLaTablaTodasLasMatriculas(PdfPTable tabla) {
	...
	public void exportarMatriculasActivas(HttpServletResponse response) throws DocumentException, IOException {
		...
		Paragraph titulo = new Paragraph("Mis matrículas activas", fuente);
		...
		tabla.setWidths(new float[] {2f, 5f, 3f, 3f, 4f, 3f});
		tabla.setWidthPercentage(110);
		
		
		escribirCabeceraDeLaTabla(tabla);
		escribirDatosDeLaTablaMatriculasActivas(tabla);
	...
	public void exportarTodasLasMatriculas(HttpServletResponse response) throws DocumentException, IOException {
		...
		Paragraph titulo = new Paragraph("Todas mis matrículas", fuente);
		titulo.setAlignment(Paragraph.ALIGN_CENTER);
		documento.add(titulo);
		...
		escribirCabeceraDeLaTabla(tabla);
		escribirDatosDeLaTablaTodasLasMatriculas(tabla);
