...
public class MatriculaController {
    ...
    public String retirarMatricula(@RequestParam("matriculaId") int matriculaId) {
        ...
        if (optionalMatricula.isPresent()) {
            ...
            LocalDate fechaActual = LocalDate.now();
            String fechaFormateada = fechaActual.format(DateTimeFormatter.ofPattern("dd-MM-yyyy"));
            retiro.setFecha(fechaFormateada);
    ...
    @GetMapping("/usuario/matriculas/exportarMatriculasActivasPDF")
    public void exportarListadoDeMatriculasActivasEnPDF(HttpServletResponse response) throws DocumentException, IOException {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String username = authentication.getName();
        Usuario usuario = userService.getUserByEmail(username);
        ...
        String valor = "attachment; filename=EscuelaGlobal_MatriculasActivas_" + usuario.getNombres() + "_" + usuario.getApellidos() + "_" + fechaActual + ".pdf";
        response.setHeader(cabecera, valor);
        List<Matricula> matriculas = interfaceMatriculaService.listarMatricula();
        MatriculasExporterPDF exporter = new MatriculasExporterPDF(matriculas, userService);
        exporter.exportarMatriculasActivas(response);
    }
    
    
    @GetMapping("/usuario/matriculas/exportarTodasMatriculasPDF")
    public void exportarListadoDeTodasMatriculasEnPDF(HttpServletResponse response) throws DocumentException, IOException {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String username = authentication.getName();
        Usuario usuario = userService.getUserByEmail(username);
        ...
        List<Matricula> matriculas = interfaceMatriculaService.listarMatricula();
        MatriculasExporterPDF exporter = new MatriculasExporterPDF(matriculas, userService);
        exporter.exportarTodasLasMatriculas(response);
    }
